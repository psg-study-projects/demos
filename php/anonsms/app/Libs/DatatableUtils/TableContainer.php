<?php
namespace App\Libs\DatatableUtils;
...
class TableContainer
{
    ...
    public function __construct(string $tablename, string $modelClass, array $colConfigs)
    {
        ...
        if ( !in_array('App\Libs\DatatableUtils\FieldRenderable', class_implements($modelClass)) ) {
            throw new \Exception('Object must implement App\Libs\DatatableUtils\FieldRenderable');
        }
        ...
        foreach ($colConfigs as $ccElem) {
            ...
            $c = [
                   ...
                   'name'   => $key,
                   'orderable'   => true, // default // %PSG : NEW %TODO move to package
               ];
            ...
            $c['query_params'] = !empty($ccElem['query_params']) && is_array($ccElem['query_params']) 
                                    ? $ccElem['query_params']
                                    : [];
// %PSG : NEW %TODO move to package
            $c['orderable'] = array_key_exists('op', $ccElem) && ('virtual_column'===$ccElem['op']) ? false : true; // disable sorting for virtual columns
            $this->_columns[] = $c;
                ...
                if ( array_key_exists($key,$meta) ) {
                    ...
                    switch ($json->op) {
                        ...
                        case 'link_to_route':
                            ...
                            if ( in_array('App\Libs\DatatableUtils\FieldRenderable', class_implements($r)) ) {
